...
Explicit proxy timeouts now bound upstream connect, write, and response-read stages. EventEdge also has a custom thread-safe in-memory LRU response cache with a 2-second TTL and 1024-entry capacity, plus process-local request coalescing to protect upstreams from cache stampedes. Request retry/failover, passive failure scoring, circuit breakers, connection pooling, metrics, and measured performance claims are not implemented yet.
